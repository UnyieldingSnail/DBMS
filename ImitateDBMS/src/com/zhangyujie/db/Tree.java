...
import com.zhangyujie.entity.DisplayField;
import com.zhangyujie.entity.Field;
import com.zhangyujie.entity.InternalNode;
import com.zhangyujie.entity.LeafNode;
import com.zhangyujie.entity.Node;
import com.zhangyujie.main.DBMS;
import com.zhangyujie.util.IOUtil;
...
//						df.setColumn(num);
	...
	public static void buildInternalNode(Select select, int n) {
		...
		for (ConditionalExpression condition : conditions) {
			if (!condition.isLeftConstant() && !condition.isRightConstant()) {// 是连接条件
				...
				while (left.isHasFather()) {
					left = left.getFather();
				}
				while (right.isHasFather()) {
	...
	private static Node buildInternal(Node left, Node right, ConditionalExpression joinCondition) {
		...
		for (DisplayField df : right.getSaveFields()) {
			 ...
			 */
			tdf.setColumn(tdf.getColumn() + step);// 将又半部分的所有保留字段所在列加step
			saveFields.add(tdf);
			if (joinCondition != null) {
...
//		start = System.currentTimeMillis();
		...
		try {
			...
			if (retainIndex.size() != 0) {
				String subLine = null;
//				System.out.println(retainIndex.size());
...
//		System.out.println("Write out use time: " + (end - start) + "ms");
//		Thread.sleep(5000);
	}
	...
	public static void creatInnerFile(InternalNode node) {
		...
		try {
			...
			if (node.isIsJoinOperation()) {//如果是自然连接
				if (leftChild.getAccountNumbers() > rightChild.getAccountNumbers()) {//右边孩子结点的记录条数少,为左孩子的表建立索引
				...
				int accountNumbers = 0;
				if (leftChild.getAccountNumbers() > rightChild.getAccountNumbers()) {//右边孩子结点的记录条数少作为驱动表
					String type = condition.getRightType();//连接表达式右属性类型
					int column = condition.getRightcolumn();//连接表达式右属性所在列
					long hashCode = 0;
					String line = null;
